					...
					if (nNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						for (int j = 0; j < upList.getLength(); j++) {
							...
							if (upNode.getNodeType() == Node.ELEMENT_NODE) {
								...
								info[j + 1] = new CardInstance.Info(attack, health, cost, level,
										skills.toArray(new SkillSpec[] {}));
							}
						}
	...
	public HashMap<String, String> loadSkillDesc() {
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				String name = eElement.getElementsByTagName("name").item(0).getTextContent().toLowerCase();
				String desc = "";
				if (eElement.getElementsByTagName("desc").getLength()!= 0)desc = eElement.getElementsByTagName("desc").item(0).getTextContent().toLowerCase();
